...
public class SysIdControls {
  ...
  public SysIdControls() {
    joy = new CommandXboxController(DriverConstants.kSysIDJoystickPort);
    initalizeTriggers();
  }

  public SysIdControls(CommandXboxController joy) {
    this.joy = joy;
    initalizeTriggers();
  }
  private void initalizeTriggers() {
    ...
    // Utility Bindings
    joy.a().whileTrue(SwerveSubsystem.getInstance().quasistaticForward());
    joy.b().whileTrue(SwerveSubsystem.getInstance().quasistaticReverse());  
    joy.x().whileTrue(SwerveSubsystem.getInstance().dynamicForward());
    joy.y().whileTrue(SwerveSubsystem.getInstance().dynamicReverse());
